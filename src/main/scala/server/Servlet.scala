...
class Servlet extends Stack {
  ...
  def mylog(body: String) {
    ...
    // val fle = fromURL(getClass.getResource(logFile))
    // val resourceLoc = getClass.getResource(logFile)
    // println(resourceLoc)
    // println(fromURL(resourceLoc))
    // println(resourceLoc.getPath())
    // println(resourceLoc.getFile())
    val writer = new FileWriter(logFile, true)


    try {
      writer.write(body + ", " + currentTime.toString + "\n")
    } catch {
